...
use SigmaPHP\Core\Router\ExceptionHandler;
    ...
    public function execute($route)
    {
        try {
            ...
            if (!isset($route['controller']) || empty($route['controller'])) {
                ...
                // otherwise will fetch directly from the $route['parameters']
                // in order , for example :
                //
                // action(Request $request, $name)
                //
                // in the is case we have 2 arguments , but actually only one of
                // them is exists , which is $name , so the array should only
                ...
                foreach ($reflection->getParameters() as $index => $parameter) {
                    ...
                    if (!$parameterIsClass) {
                        ...
                        // we put that value , otherwise we check for the
                        // default value from the Reflection class , if not we
                        // put null !
                ...
                // in future when the architecture will be revamped , this
                // variable will play key role !!
